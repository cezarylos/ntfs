  ...
  if (req.method === 'POST') {
    ...
    try {
      const [holderTicketResponse, eventResponse] = await Promise.all([
        ...
        StrapiService.getEventById(eventId, ['contractAddress', 'ABI', 'chainId', 'amountOfTokensToGetReward', 'excludedAddressesFromRewards'])
      ]);

      const { meta: { pagination: { total } } } = holderTicketResponse;
      const {
        contractAddress,
        ABI,
        chainId,
      ...
      const tokensIds = (await contract.methods.getTokensByOwner?.(address).call())?.map((tokenId: string) =>
        Number(tokenId)
      ) || [];
      ...
      const emptyTicketsResponse = await StrapiService.getTicketsWithoutHolderAddress(process.env.STRAPI_API_TOKEN as string);
      ...
      await StrapiService.assignHolderAddressToTicket(process.env.STRAPI_API_TOKEN as string, emptyTicketId, address.toLowerCase());
